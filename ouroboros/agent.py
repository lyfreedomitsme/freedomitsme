...
class OuroborosAgent:
    ...
    def __init__(self, env: Env, event_queue: Any = None):
        ...
        # SSOT modules
        # Initialize LLMClient with actual remaining budget from state.json
        budget_remaining = self._read_remaining_budget()
        self.llm = LLMClient(budget_remaining=budget_remaining)
    ...
    def _read_remaining_budget(self) -> float:
        """Read remaining budget from state.json and TOTAL_BUDGET env var."""
        try:
            ...
            if not total_budget_str:
                # No TOTAL_BUDGET set, default to 1000.0
                return 1000.0
            ...
            log.warning(f"Failed to read budget from state: {e}", exc_info=True)
